...
let x = function(p){
    ...
    return (p.split('')).map(j1).map(j2).reduce(j3)
}
...
console.log(`O Texto possui ${(klingon.map(x).filter(j4).filter(j5)).length} números bonitos!`)
